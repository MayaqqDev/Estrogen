...
public class CentrifugingCategory extends CreateRecipeCategory<CentrifugingRecipe> {
    ...
    public List<Widget> setupDisplay(CreateDisplay<CentrifugingRecipe> display, Rectangle bounds) {
        ...
        Slot fluidInputSlot = basicSlot(xInput, y, origin).markInput().entries(EntryIngredients.of(CreateRecipeCategory.convertToREIFluid(inputStack)));
        widgets.add(fluidInputSlot);

        Slot fluidOutputSlot = basicSlot(xOutput, y, origin).markInput().entries(EntryIngredients.of(CreateRecipeCategory.convertToREIFluid(outputStack)));
